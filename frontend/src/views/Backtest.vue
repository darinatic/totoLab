...
const option = computed(() => {
  ...
  if (!bt.value) return {}
  const t = tokens()
  const rows = [...bt.value.results].reverse() // best at top
  const cats = rows.map((r) => r.label)
  return {
    ...baseOption(t),
    grid: { left: 130, right: 30, top: 20, bottom: 40, containLabel: true },
    tooltip: {
      ...baseOption(t).tooltip, trigger: 'item',
      formatter: (p) => {
        const r = rows[p.dataIndex]
        return `${r.label}<br/>mean matches: <b>${r.mean_matches}</b>` +
          `<br/>95% CI: ${r.ci_low}–${r.ci_high}` +
          (r.p_vs_random != null ? `<br/>p vs random: ${r.p_vs_random}` : '')
      },
    },
    xAxis: {
      type: 'value', name: 'avg. matched numbers (of 6)', nameLocation: 'middle',
      nameGap: 26, nameTextStyle: { color: t.muted, fontSize: 11 },
    ...
    series: [
      {
        type: 'bar',
        data: rows.map((r) => ({
          value: r.mean_matches,
          itemStyle: { color: r.strategy === 'random' ? t.series[2] : t.series[0],
            borderRadius: [0, 3, 3, 0] },
        })),
        barWidth: 18,
        markLine: {
          silent: true, symbol: 'none',
          data: [{ xAxis: bt.value.theoretical_random }],
          lineStyle: { color: t.critical, type: 'dashed', width: 2 },
          label: { formatter: 'theoretical random\n{c}', color: t.critical,
      ...
      },
      {
        type: 'custom',
        encode: { x: [0, 1], y: 2 },
        renderItem: (params, apiObj) => {
          const lo = apiObj.coord([apiObj.value(0), apiObj.value(2)])
          const hi = apiObj.coord([apiObj.value(1), apiObj.value(2)])
          const y = lo[1]
          const cap = 5
          const style = { stroke: t.text, lineWidth: 1.5 }
          const line = (x1, y1, x2, y2) => ({ type: 'line', shape: { x1, y1, x2, y2 }, style })
          return {
            type: 'group', children: [
              line(lo[0], y, hi[0], y),
              line(lo[0], y - cap, lo[0], y + cap),
              line(hi[0], y - cap, hi[0], y + cap),
            ],
          }
        },
        data: rows.map((r, i) => [r.ci_low, r.ci_high, i]),
        z: 10,
      },
    ],
  }
...
<template>
  <div class="container">
    ...
    <p class="page-sub">
      The honest heart of this project. We walk forward through history: at each past
      draw we predict using <em>only</em> the draws before it, then score the hit.
      If any strategy had real skill, its bar would sit clearly right of the red line.
    </p>

    <div class="controls">
      <label class="hint">Test window
        <input class="number-input" type="number" v-model.number="testSize" min="20" max="500" step="10" />
      </label>
    ...
    <template v-else>
      ...
      <div class="card">
        <h3>Average matched numbers per strategy</h3>
        <p class="card-sub">
          Bars show the mean over {{ bt.test_size }} test draws; whiskers are the 95%
          bootstrap confidence interval. Random-baseline theory: {{ bt.theoretical_random }}.
        </p>
      ...
      <div class="card" style="margin-top: 16px">
        ...
        <table class="data">
          <thead>
            <tr><th>Strategy</th><th>Mean</th><th>95% CI</th><th>Hit ≥3</th><th>p vs random</th><th>Verdict</th></tr>
          </thead>
          <tbody>
            <tr v-for="r in bt.results" :key="r.strategy">
              ...
              <td>{{ (r.hit_rate_3plus * 100).toFixed(1) }}%</td>
              <td>{{ r.p_vs_random ?? '—' }}</td>
              <td>
                <span class="badge" :class="r.beats_random ? 'bad' : 'good'">
                  {{ r.beats_random ? 'above (noise)' : 'no edge' }}
                </span>
              </td>
            </tr>
          </tbody>
        ...
        <table class="data">
          ...
          <tbody>
            <tr v-for="test in fairness.tests" :key="test.test">
              <td>{{ test.test }}</td>
              <td>{{ test.p_value ?? '—' }}</td>
              <td><span class="badge" :class="test.passes ? 'good' : 'bad'">
                {{ test.passes ? 'pass' : 'flag' }}</span></td>
